...
def update_suggestions(*args):
    ...
    thread = threading.Thread(target=search_and_update, args=(keyword, folder_path))
    thread.start()

def search_and_update(keyword, folder_path):
    suggested_files = suggest_files(keyword, folder_path, max_results=20)
    filtered_files = filter_files(suggested_files, keyword)

    # Update the list of suggestions in the main GUI thread
    lb.delete(0, tk.END)
...
lb = tk.Listbox(list_frame, width=80, height=20, font=("Calibri", 12), yscrollcommand=yscrollbar.set,
